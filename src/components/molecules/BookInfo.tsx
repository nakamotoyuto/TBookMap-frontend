...
export const BookInfo = ({ book }: Props) => {
  ...
  return (
    <div className='flex justify-start mt-10 mb-10 gap-28'>
      <Image loader={myLoader} src={`${image_url}`} width={225} height={300} layout="intrinsic" alt={title}/>
      <div className='text-lg'>
        {detail && <p className='inline-block pb-4 border border-solid border-zinc-700'>{detail}</p> }
        <div className='flex flex-col gap-5 pt-4'>
          <p>出版社 : { publisher_name }</p>
          <p>著者・編集 : { author }</p>
          <p>詳細リンク : <Link href={ detail_url }>{ detail_url }</Link></p>
        </div>
      </div>
    </div>
  );
